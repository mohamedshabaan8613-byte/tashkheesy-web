...
export function useNavigationRecovery(): UseNavigationRecoveryReturn {
  ...
  useEffect(() => {
    ...
    const phase = getRecoveryPhase(hydration as unknown as import('../types/consultationTypes').ConsultationNavigationState, window.location.pathname);

    if (hydration.wasRecovered || hydration.source === "url") {
      setRecoveryState({
        wasRecovered: true,
        recoverySource: "refresh",
        currentPhase: (phase ?? "IDLE") as string,
      });
    }
  }, []);
  ...
  useEffect(() => {
    function handlePopState() {
      ...
      if (nextPhase === "IDLE" && currentPhase !== "IDLE") {
        const result = transition(currentPhase as Parameters<typeof transition>[0], "EXITED");
        if (result.success) {
          clearIntent();
        }
      }
      ...
      setRecoveryState((prev) => ({
        ...
        currentPhase: (nextPhase ?? "IDLE") as string,
      }));
    }
