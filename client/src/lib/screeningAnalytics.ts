...
export class FunnelSession {
  ...
  /** R8 FIX: lightweight no-op analytics hook for field blur events.
   * Prevents TypeError in AssessmentForm.tsx when funnelSession.onFieldBlur() is called.
   * Intentionally no-op — field-level blur tracking is not yet wired to Supabase. */
  onFieldBlur(_fieldName: string): void {
    // no-op: safe compatibility stub — do not add analytics logic here without
    // first adding the corresponding column to the screening_analytics table.
  }
