...
function isValidTransition(
  ...
  if (TERMINAL_PHASES.includes(from)) return false;
...
export function createConsultationBookingRepository() {
  ...
  function updatePhase(
    ...
    extras?: Partial<Pick<
      ConsultationBookingSession,
      "selectedSpecialistId" | "selectedSlotId"
    >>
   ...
   * شروط الاستعادة:
   *   • توجد جلسة مخزّنة
   *   • حالتها قابلة للاستعادة (RECOVERABLE_PHASES)
  ...
  function recover(
    ...
    if (!RECOVERABLE_PHASES.includes(persisted.bookingStatus)) return null;
    ...
    _session = {
      ...
      recoveryState: {
        status: "recovered",
        reason: reason,
        recoveredAt: persisted.lastActivityAt,
        recoveredPhase: persisted.bookingStatus,
      },
    };
