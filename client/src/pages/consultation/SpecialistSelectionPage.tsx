...
function RecoveryScreen({ reason, onRetry }: RecoveryScreenProps) {
  ...
  const content = {
    expired: {
      ...
      cta: "البدء من جديد",
      action: () => navigate("/consultation/start"),
    },
    missing: {
...
function EmptySpecialistsState() {
  ...
  return (
    <div className="text-center py-16 px-4 space-y-4">
      ...
      <button
        onClick={() => navigate("/consultation/start")}
...
export default function SpecialistSelectionPage() {
  ...
  useRuntimeSafetyCheck(
    session ?? undefined,
    undefined,
    (violation) => setSafetyViolation(violation.type)
  );
  ...
  if (ownershipGuard.status === "checking" || hydration.status === "checking") {
    return <HydrationLoadingScreen />;
  }

  if (ownershipGuard.status !== "OWNED") {
    const reason =
      ownershipGuard.status === "EXPIRED" ? "expired" :
      ownershipGuard.status === "NOT_FOUND" ? "missing" : "ownership_failed";
  ...
  if (hydration.status === "missing") {
    return <RecoveryScreen reason="missing" onRetry={() => navigate("/consultation/start")} />;
  }
