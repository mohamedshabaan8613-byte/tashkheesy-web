...
export default function Step3DataConfirmation() {
  ...
  const handleSubmit = () => {
    ...
    if (!formData.parentName || !formData.parentEmail || !formData.parentPhone) {
      toast.error("يرجى ملء البيانات الشخصية", {
        description: "تأكد من إدخال الاسم والبريد الإلكتروني ورقم الجوال للمتابعة.",
      });
      return;
    }

    if (!formData.patientName || !formData.patientAge) {
      toast.error("يرجى ملء بيانات الطفل/الطالب", {
        description: "الاسم والعمر حقلان إلزاميان للمتابعة.",
      });
      return;
    }

    if (!formData.privacyAgreed || !formData.disclaimerAgreed) {
      toast.error("يرجى قبول الشروط", {
        description: "يجب الموافقة على سياسة الخصوصية وإخلاء المسؤولية قبل المتابعة.",
      });
      return;
    }

    if (isChildService && !formData.guardianAgreed) {
      toast.error("يرجى تأكيد صفة ولي الأمر", {
        description: "يجب تأكيد أنك ولي أمر الطفل أو مخوّل قانونياً لطلب هذا التشخيص.",
      });
      return;
    }
