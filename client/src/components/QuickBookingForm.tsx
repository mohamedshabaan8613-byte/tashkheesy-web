    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.success("تم إرسال طلبك بنجاح!", {
          description: "سنتواصل معك خلال 24 ساعة لتأكيد موعدك.",
          duration: 3000,
        });

        // إعادة توجيه بعد 3 ثوان باستخدام wouter (SPA-safe)
        setTimeout(() => navigate("/booking"), 3000);
      } else {
        const errMsg = "حدث خطأ في إرسال النموذج. يرجى المحاولة مرة أخرى.";
        setError(errMsg);
        toast.error("فشل الإرسال", { description: errMsg });
      }
    } catch (err) {
      const errMsg = "حدث خطأ في الاتصال. يرجى التحقق من الاتصال بالإنترنت.";
      setError(errMsg);
      toast.error("تعذّر الاتصال", { description: errMsg });
    } finally {
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <p className="text-xs text-slate-500 text-center">
            ...
            <Link href="/privacy">
              <a className="text-[#1E4E8C] hover:underline">سياسة الخصوصية</a>
            </Link>
          </p>
        </form>
      </CardContent>
